// Waska_PN_A_Rev33_AC-DC_BatRuleAllowsAmps.ino
//
// Rev33 AC/DC:
...
// Gate thresholds (on total RMS current)
...
HardwareSerial &LOGSERIAL = Serial0;
...
void readClampStats(float &meanCounts, float &rmsCounts) {
  uint32_t acc = 0;
  ...
  float rms = sqrt(sumSq / (double)AMP_N_SAMPLES);
  meanCounts = mean;
  rmsCounts = rms;
}
...
void drawOLED(float V, float Idc, float Iac) {
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_6x12_tf);
  u8g2.drawStr(0, 10, "Waska-PN A Rev33");
  ...
  char l4[24];
  snprintf(l4, sizeof(l4), "AC %5.2fA", Iac);
  u8g2.drawStr(0, 60, l4);
...
void setup() {
  initSerial();
  LOGSERIAL.println("BOOT: Waska_PN_A Rev33 AC/DC BatRuleAllowsAmps");
  ...
  oledHardResetPulse();
  u8g2.begin();
  drawOLED(0.0f, 0.0f, 0.0f);

  LOGSERIAL.print("Boot settle "); LOGSERIAL.print(BOOT_SETTLE_MS); LOGSERIAL.println(" ms...");
  delay(BOOT_SETTLE_MS);

  LOGSERIAL.println("Measuring clamp noise floor (BNC connected, clamp NOT clamped)...");
  float clampMeanCounts = 0.0f;
  float clampRmsCounts = 0.0f;
  readClampStats(clampMeanCounts, clampRmsCounts);
  ClampZeroCounts = clampMeanCounts;
  NoiseFloorA = countsToAmps(clampRmsCounts);
  LOGSERIAL.print("ClampZeroCounts: "); LOGSERIAL.println(ClampZeroCounts, 2);
  LOGSERIAL.print("NoiseFloorA (A): "); LOGSERIAL.println(NoiseFloorA, 3);
  ...
  LOGSERIAL.print("Rev33: Bat rule only forces Gate OFF if V<");
  LOGSERIAL.print(V_BAT_PRESENT_MIN, 2);
  LOGSERIAL.print(" AND Itotal<");
...
void loop() {
  ...
  if (now - lastSenseMs >= SENSE_INTERVAL_MS) {
    ...
    float clampRmsCounts = 0.0f;
    readClampStats(clampMeanCounts, clampRmsCounts);
    float Idc = countsToAmps(clampMeanCounts - ClampZeroCounts);
    float Iac = countsToAmps(clampRmsCounts);

    // Compute Inet (AC, noise-floor-adjusted) first (Rev22 needs this before the battery rule)
    float Inet = Iac - NoiseFloorA;
    if (Inet < 0.0f) Inet = 0.0f;

    float Itotal = sqrt((Idc * Idc) + (Inet * Inet));

    // Rev22 battery removed rule:
    // Only force gate OFF when battery/VSENSE is absent AND current is also basically zero.
    if (V < V_BAT_PRESENT_MIN && Itotal <= BAT_REMOVED_I_MAX_A) {
      gateOn = false;
      hitsOn = 0;
      hitsOff = 0;
    }
    ...
    if (!gateOn && Inet <= ADAPT_INET_MAX_A) {
      NoiseFloorA = (1.0f - NOISE_ALPHA) * NoiseFloorA + (NOISE_ALPHA) * Iac;
    }
  ...
  if (now - lastBeat >= BEAT_INTERVAL_MS) {
    ...
    LOGSERIAL.print(" Iac="); LOGSERIAL.print(lastIac, 2);
  ...
  if (now - lastOledMs >= OLED_INTERVAL_MS) {
    lastOledMs = now;
    drawOLED(lastV, lastIdc, lastIac);
  }
  ...
  if (loraOK && (now >= nextTxMs)) {
    ...
    snprintf(packet, sizeof(packet),
             "P,A,RUN,%.2f,%.2f,%.2f,%.2f,%d",
             lastV, lastIdc, lastIac, Vditch, Rindex);
